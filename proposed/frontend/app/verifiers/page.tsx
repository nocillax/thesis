...
import { formatDateTime, formatIpAddress } from "@/lib/utils/format";
    ...
    <div className="container py-8">
      ...
      <Card>
        ...
        <CardContent>
              ...
              <TableBody>
                {logsData.data.map((log: VerificationLog) => (
                  <TableRow key={log.id}>
                    ...
                    <TableCell>{log.verifier.institution}</TableCell>
                    <TableCell className="font-mono text-sm">
                      {formatIpAddress(log.ip_address)}
                    </TableCell>
                    <TableCell className="font-mono text-xs">
                      {log.cert_hash.slice(0, 16)}...
                    </TableCell>
                    ...
                    <TableCell>
                      <Dialog
                        open={blockDialogOpen && selectedIp === log.ip_address}
                        onOpenChange={(open) => {
                      ...
                      >
                        ...
                        <DialogContent>
                          <DialogHeader>
                            ...
                            <DialogDescription>
                              Block {log.ip_address} from verifying certificates
                            </DialogDescription>
                          </DialogHeader>
          ...
          {logsData && logsData.meta && logsData.meta.total_pages > 1 && (
            <div className="flex items-center justify-between mt-4 pt-4 border-t">
              <p className="text-sm text-muted-foreground">
                Page {logsData.meta.current_page} of {logsData.meta.total_pages}{" "}
                ({logsData.meta.total_count} total logs)
              </p>
